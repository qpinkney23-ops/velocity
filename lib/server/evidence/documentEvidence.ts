...
export type DocumentEvidenceSetV1 = Readonly<{ schemaVersion: typeof DOCUMENT_EVIDENCE_SET_V1; documentId: string; applicationId: string; tenantId: string; documentAuthorizationVersion:string; sourceContentHash:string; evidence: readonly DocumentEvidenceRecordV1[]; summary: Readonly<{ evidenceCounts: Readonly<Record<EvidenceType, number>>; warnings: readonly string[]; conflicts: readonly EvidenceConflictV1[]; missingCategories: readonly EvidenceType[]; processingFingerprint: string }> }>;
...
const rules: readonly Readonly<{ regex: RegExp; evidenceType: EvidenceType; fieldIdentifier: string; kind: NormalizedEvidenceValueV1["kind"] }>[] = Object.freeze([
  ...
  { regex: /(?:property value|appraised value)\s*:\s*(\$?[\d,]+(?:\.\d{1,2})?)/gi, evidenceType: "property", fieldIdentifier: "property.value", kind: "property_value" },
  { regex: /(?:purchase price|contract price|purchase agreement price)\s*:\s*(\$?[\d,]+(?:\.\d{1,2})?)/gi, evidenceType: "purchase_contract", fieldIdentifier: "purchase_contract.price", kind: "currency" },
